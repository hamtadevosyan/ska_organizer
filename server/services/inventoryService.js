...
const snapshot = (item) => ({ ...Object.fromEntries(['id', 'name', 'category', 'location', 'unit', 'ingredientId', 'version'].map((key) => [key, item[key]])),
  quantity: v.decimal(v.amount(item.quantity)), reorderThreshold: v.decimal(v.amount(item.reorderThreshold)) });
const summary = (item, ingredient) => ({ ...item, ...snapshot(item),
...
async function present(item) { return summary(item, item.ingredientId ? await db.getIngredientById(item.ingredientId) : null); }
...
async function replay(payload, operation, itemId, actor) {
  ...
  if (!old) return null;
  if (!isDeepStrictEqual(old.request, signature(payload, operation, itemId, actor))) throw Object.assign(problem('This request identifier was already used for different inventory details. Reload before trying again.', 409), { code: 'INVENTORY_REQUEST_CONFLICT' });
...
exports.update = (id, payload, actor) => locked(async () => {
  ...
  const values = v.item(payload, previous);
  v.text(payload.reason, 'reason', 500);
...
exports.getInventoryItems = (query = {}) => db.withInventoryLock(async () => {
  ...
  const [items, total, status, options, ingredients] = await Promise.all([
    db.listInventory(filters), db.countInventory(filters), exports.getStatus(), db.inventoryOptions(), db.listIngredients({ includeArchived: true }),
  ]);
  const byId = new Map(ingredients.map((item) => [item.id, item]));
  return { items: items.map((item) => summary(item, byId.get(item.ingredientId))), total,

...
const labels = { opening: 'Starting amount', addition: 'Added', usage: 'Used', correction: 'Amount checked', details: 'Details changed' };
  ...
  return <section aria-label="Inventory history" className="space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
    ...
    {!loading && !error && data && <>
      ...
      <ol className="space-y-3">{data.items.map((movement) => <li key={movement.id} className="rounded-lg border p-4">
        <div className="flex flex-wrap justify-between gap-2"><h3 className="font-semibold">{labels[movement.type]}</h3></div>
